...
local function res_user_callback(extra, success, result) -- /infull <username> function
  if success == 1 then  
...
local function action_by_id(extra, success, result)  -- /infull <ID> function
 if success == 1 then
 if result.username then
   Username = '@'..result.username
   else
  ...
  send_msg(extra.receiver, 'id not found.\nuse : /infull @username', ok_cb, false)
  end
end

local function action_by_reply(extra, success, result)-- (reply) /infull  function
...
local function run(msg, matches)
 ...
 if matches[1]:lower() == 'infull' and not matches[2] then
  local receiver = get_receiver(msg)
  local Reply = msg.reply_id
  if msg.reply_id then
  ...
  if matches[1]:lower() == 'infull' and matches[2]  then
   local user = matches[2]
   local chat2 = msg.to.id
   local receiver = get_receiver(msg)
   if string.match(user, '^%d+$') then
	  user_infull('user#id'..user, action_by_id, {receiver=receiver, user=user, text=text, chat2=chat2})
...
return {
  description = 'Know your infullrmation or the info of a chat members.',
  usage = {
	'!infull: Return your info and the chat info if you are in one.',
	'(Reply)!info: Return info of replied user if used by reply.',
	'!infull <id>: Return the info\'s of the <id>.',
	'!infull @<user_name>: Return the member @<user_name> information from the current chat.',
